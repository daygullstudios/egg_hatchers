...
The major game screens now share a persistent navigation shell with Hatchery, Shop, Battles, Collection, and More. The navigation and every app-bar bottom slot are hard-limited to the same 430px centered portrait boundary as the game content, including on desktop web; regression tests protect that constraint. Tabs remain mounted so scroll and screen state survive switching. The Quests screen uses a single-open accordion, a pinned category jump control, a unified Ready to Claim section with Claim All for ordinary rewards, intelligent progress sorting, and hidden claimed quests.
